...
Logger::Logger() { _lastLogTime = 0; }

void Logger::begin() {
    if (LOG_ENABLED) Serial.println("📋 Логгер инициализирован");
}
...
void Logger::logf(const char* category, const char* format, ...) {
    if (!LOG_ENABLED) return;
    Serial.print("["); Serial.print(category); Serial.print("] ");
    va_list args;
...
void Logger::logSensors(float temp, float hum, float dsTemp, float light) {
    ...
    if (LOG_DETAILED)
        logf("ДАТЧИКИ", "AHT20: %.1f°C %.1f%% | DS18B20: %.1f°C | BH1750: %.0f lx", temp, hum, dsTemp, light);
    else
...
void Logger::logRelay(bool lamp1, bool lamp2, bool lamp3, bool humidifier, int mode) {
    ...
    if (LOG_DETAILED)
        logf("РЕЛЕ", "L1=%d L2=%d L3=%d H=%d | Режим: %s", lamp1, lamp2, lamp3, humidifier, modeStr);
    else
...
void Logger::logSchedule(const char* msg) {
    if (!LOG_ENABLED || !LOG_SCHEDULE) return;
    log("РАСПИСАНИЕ", msg);
}
